...
import time
#####################初始化#################################
Camera=CameraCapture(width=640,height=640,fps=120)
Serial=Ser(baudrate_=9600,timeout_=1,port_='/dev/ttyUSB0')

Pid_X = PID(Kp=19.82, Ki=0.25, Kd=3.5, output_limit=30, integral_limit=20)
Pid_Y = PID(Kp=21.82, Ki=0.25, Kd=3.5, output_limit=30, integral_limit=20)

vel_pid_x = PID(Kp=19, Ki=0, Kd=15)
vel_pid_y = PID(Kp=19, Ki=0, Kd=15)

Ball=BallPIDController(Pid_X,Pid_Y,vel_pid_x,vel_pid_y,stay_duration=5)
...
######################任务选择###############################
        if(Serial.task=='task1'):
            ...
            task_flag=2
            Ball.stay_duration=5
            Ball.set_target_region(6)
            angle_x,angle_y=Ball.update(x_pos,y_pos)
            Serial.Servo_set_angle(angle_x,angle_y,-angle_x,-angle_y)


        elif(Serial.task=='task3'):
            Ball.stay_duration=3
            if task_flag<3 or task_flag>4:
                task_flag=3.1
            ...
            if task_flag<4 or task_flag>5:
                task_flag=4.1
                Ball.stay_duration=0.5
            ...
            if index >= len(task4_path):
                index=len(task4_path)-1
                Ball.stay_duration=3
...
#######################显示内容#############################
        #绘制任务进程
        cv2.putText(frame,f'task:{task_flag},ball:{ball_regin}',(0,10),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
        cv2.imshow("ball_detect", frame)
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            break
